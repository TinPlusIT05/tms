...
class quality_test(base_module_quality.abstract_quality_check):
    ...
    def __init__(self):
        ...
        self.note = _("PEP-8 Test, copyright of py files check, method"
                      " can not call from loops")
    ...
    def run_test(self, cr, uid, module_path):
        ...
        for file_py in list_files:
            if file_py.split('.')[-1] == 'py' and not file_py.endswith(
                    '__init__.py') and not file_py.endswith('__openerp__.py'):
        ...
        # 2. Imports are always put at the top of the file, just after any
        ...
        # TODO: JCD 2013-04-03 we will do this in the future:
        # self.check_licence(open_files)

        # 1. Does the module avoid unecessary queries like when we put a browse
        # into a loop?
        self.check_loop(open_files)

        # 1.More than one space around an assignment (or other) operator to
        # align it with another.
        self.check_space_operator(open_files)

        # 1. For sequences, (strings, lists, tuples),
        ...
        self.score = self.good_standard and float(
            self.good_standard) / float(self.good_standard + self.bad_standard)
        ...
        for py in open_files:
            ...
            while True:
                ...
                if ((line.find('class ') > -1) or (line.find('def ') > -1)):
                    class_or_def = True
                if (line.find("_('") > -1 or line.find('_("') > -1):
                    raise_warning = True

                if (line.find('import') > -1 and line.find('_') > -1):
                    import_raise_found = True
            ...
            if raise_warning and not import_raise_found:
                self.bad_standard += 1
                self.result_py[file_name] = [file_name, 0, 'Please Import _ ']
            ...
            while True:
                ...
                if (comment_found > -1) and bad_position and declaration:
                    ...
                    self.result_py[
                        ...
                        file_name,
                        line_counter,
                        'Declaration of copyright must be at the top of file']
                    break
            if bad_position and (not flag):
                self.bad_standard += 1
                self.result_py[file_name] = [
                    file_name, '--', 'File is not copyright']
    ...
    def check_loop(self, open_files):
        for py in open_files:
            ...
            while True:
                ...
                if not line:
                    break
                place_for = line.find('for ')
                if place_for > -1:
                    counter += 1
                    for method in methods:
                        place_method = line.find(method)
                        if place_method > -1 and place_method > place_for:
                            self.bad_standard += 1
                            self.result_py[
                                file_name +
                                str(line_counter)] = [
                                file_name,
                                line_counter,
                                'puting method inside loop is not good']
            ...
            while True:
                ...
                if (pos_comma != -1 or pos_semicolon != -1 or pos_colon != -1):
                    ...
                    for i in line:
                        space_find += 1
                        if (i == ' '):
                            if ((space_find + 1) == pos_comma) or\
                                    ((space_find + 1) == pos_semicolon) or\
                                    ((space_find + 1) == pos_colon):
                                self.bad_standard += 1
                                self.result_py[
                                    file_name +
                                    str(counter_line)] = [
                                    file_name,
                                    counter_line,
                                    'You should not have space before (: ; ,)']
    ...
    def check_space_operator(self, open_files):
        for py in open_files:
            ...
            while True:
                ...
                if not line:
                    break
                value_before_space = False
                for counter in line:
                    if (counter == ' '):
                        space_counter += 1
                    else:
                        if space_counter > 1 and value_before_space:
                            if counter in [
                                    '=',
                                    '<',
                                    '>',
                                ...
                                self.result_py[
                                    ...
                                    '(or other) operator to align it with '
                                    'another']
                        operator_found = False
                        space_counter = 0
                        value_before_space = True
                    if counter in [
                            '=',
                            '<',
                            '>',
                ...
                if (line.find('if') > -
                    ...
                    if (line.find(':') > -1) and not line.find('<') > -1\
                        ...
                        self.result_py[
                            ...
                            file_name,
                            line_counter,
                            ' For sequences, (strings, lists, tuples), use the'
        ...
        for py in open_files:
            ...
            while True:
                ...
                if (line.find('if') > -1):
                    ...
                    # This is to accept the case with:
                    # status.get('uncovered_modules', False)
                    if (line.find('.get(') == -
                        1 and line.find('False)') == -
                        1 and line.find('True)') == -
                        1) and ((line.find('==') > -
                                 1) or (line.find('!=') > -
                                        1)) and ((line.find('True') > -
                                                  1) or (line.find('False') > -
                                                         1)):
                        self.bad_standard += 1
                        self.result_py[
                            file_name +
    ...
    def get_result_general(self, dict_obj):
        str_html = '<html><strong>Result</strong><head>%s</head>' \
                   '<body><table class="tablestyle">' % (self.get_style())
        header = (
            '<tr><th class="tdatastyle">%s</th><th class="tdatastyle">%s</th>'
            '<th class="tdatastyle">%s</th></tr>',
            [
                _('Object Name'),
                _('Line number'),
                _('Suggestion')])
