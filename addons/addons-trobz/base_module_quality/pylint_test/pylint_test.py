...
class quality_test(base_module_quality.abstract_quality_check):
    ...
    def __init__(self):
        ...
        self.note = _(
            "This test uses Pylint and checks if the module satisfies "
            "the coding standard of Python. See "
            "http://www.logilab.org/project/name/pylint for further info.\n ")
        ...
        for file_py in list_files:
            if file_py.split('.')[-1] == 'py'\
                ...
                try:
                    ...
                    self.error = True
                    self.log.exception(
                        "Cannot run pylint test for %s", file_path)
                ...
                try:
                    ...
                    self.log.warning(
                        "Cannot parse pylint result", exc_info=True)
                    score += 0
                    dict_py[file_py] = [file_py, _("Unable to parse the result"
                                                   ". Check the details.")]
                replace_string = ''
                replace_string += res
