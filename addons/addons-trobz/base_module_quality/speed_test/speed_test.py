...
class quality_test(base_module_quality.abstract_quality_check):
    ...
    def __init__(self):
        ...
        self.note = _("This test checks the speed of the module. "
                      "Note that at least 5 demo data is needed in "
                      "order to run it.")
        self.min_score = 30
    ...
    def run_test(self, cr, uid, module_path):
        ...
        result_dict = {}
        result_dict2 = {}
        self.result_details += _(
            "<html>O(1) means that the number of SQL requests to read the "
            "object does not depand on the number of objects we are reading. "
            "This feature is mostly wished.\n</html>")
        ...
        for obj, ids in obj_ids.items():
            ...
            if size:
                ...
                try:
                    ...
                    registry.get(obj).read(cr, uid, ids)

                    ccr.reset()
                    registry.get(obj).read(ccr, uid, [ids[0]])
                    code_base_complexity = ccr.count

                    ccr.reset()
                    registry.get(obj).read(ccr, uid, ids[:size / 2])
                    code_half_complexity = ccr.count

                    ccr.reset()
                    registry.get(obj).read(ccr, uid, ids)
                    ...
                    speed_list = [obj, size, code_base_complexity,
                                  code_half_complexity, code_size_complexity,
                                  _("Error in Read method: %s") % e]
                else:
                    if size < 5:
                        speed_list = [obj, size, code_base_complexity,
                                      code_half_complexity,
                                      code_size_complexity,
                                      _("Warning! Not enough demo data")]
                        list2 = [obj, _("No enough data")]
                    else:
                        if code_size_complexity <= (code_base_complexity
                                                    + size):
                            complexity = _("O(1)")
                            score += 1
                        ...
                        speed_list = [
                            obj, size, code_base_complexity,
                            code_half_complexity, code_size_complexity,
                            complexity]
            else:
                speed_list = [obj, size, "", "", "",
                              _("Warning! Object has no demo data")]
                list2 = [obj, _("No data")]
        ...
        #     self.message = _(
        #         'Score is below than minimal score(%s%%)') % self.min_score
    ...
    def get_result_details(self, dict_speed):
        ...
        header = ('<tr><th class="tdatastyle" >%s</th><th class="tdatastyle">'
                  '%s</th><th class="tdatastyle">%s</th>'
                  '<th class="tdatastyle">%s</th><th class="tdatastyle">%s'
                  '</th><th class="tdatastyle">%s</th></tr>',
                  [_('Object Name'), _('N (Number of Records)'), _('1'),
                   _('N/2'), _('N'), _('Reading Complexity')])
        if not self.error:
            res = str_html + \
                self.format_html_table(
                    header, data_list=dict_speed) + '</table></body></html>'
