...
class quality_test(base_module_quality.abstract_quality_check):
    ...
    def run_test_struct(self, cr, uid, module_path):
        ...
        if not module_name.islower():
            self.result_dict[module_name] = [
                module_name, 'Module name should have in lowercase']
        for file_struct in list_files:
            if file_struct.split('.')[-1] != 'pyc':
                ...
                if file_struct == 'wizard' and os.path.isdir(path):
                    ...
                    module_dict[file_struct] = []
                elif file_struct != 'views' and os.path.isdir(path):
                    module_dict[file_struct] = []
                elif file_struct != 'controllers' and os.path.isdir(path):
                    module_dict[file_struct] = []
        ...
        for i in f_list:
            ...
            if os.path.isdir(path) and i != 'i18n':
                for j in os.listdir(path):
                    if i in ['report', 'wizard', 'security', 'module',
        ...
        com_list = ['_unit_test.xml', '.py', '_view.xml', '_workflow.xml',
                    '_wizard.xml', '_report.xml', '_data.xml', '_demo.xml',
                    '_security.xml', '_sequence.xml', '_graph.xml']
        com_list = map(lambda x: module_name + x, com_list)
        main_file = ['__init__.py', '__openerp__.py']
        com_list.extend(main_file)
        module_dict['module'] = filter(
            lambda x: len(x.split(".")) > 1, module_dict['module'])
        score = self.get_score(module_dict['module'], com_list)
        self.count += 1
        final_score += score

        # report folder checking...
        if 'report' in module_dict:
            # Find all of the files in sub-folders in folder 'report'
            #    except __ini__.py and .pyc files
            for file_struct in module_dict['report']:
                path = os.path.join(module_path, 'report/' + file_struct)
                if os.path.isdir(path):
                    for j in os.listdir(path):
                        if j != '__init__.py' and j.split('.')[-1] != 'pyc':
                            module_dict['report'].append(file_struct + '/' + j)

            report_pys = filter(lambda x: (len(x.split('.')) > 1 and x.split(
                '.')[1] == 'py') and x != '__init__.py', module_dict['report'])
            report_pys = map(lambda x: x.split('.')[0], report_pys)
            reports = ['.sxw', '.rml', '.xsl', '.py', '.xml']

            # Remove sub-folder in folder 'report'
            module_dict['report'] = filter(lambda x: '.' in x,
                                           module_dict['report'])
            ...
            score_report = self.get_score(
                module_dict['report'], org_list_rep, 'report/')
            
            # In some case we only need to overwrite an XML file
            self.count = self.count + 1
            final_score += score_report

        # wizard folder checking...
        if 'wizard' in module_dict:
            # Find all of the files in sub-folder in folders 'wizard'
            #    except __ini__.py and .pyc files
            for file_struct in module_dict['wizard']:
                path = os.path.join(module_path, 'wizard/' + file_struct)
                if os.path.isdir(path):
                    for j in os.listdir(path):
                        if j != '__init__.py' and j.split('.')[-1] != 'pyc':
                            module_dict['wizard'].append(file_struct + '/' + j)

            wizard_pys = filter(lambda x: (len(x.split('.')) > 1 and x.split(
                '.')[1] == 'py') and x != '__init__.py', module_dict['wizard'])
            wizard_pys = map(lambda x: x.split('.')[0], wizard_pys)
            wizards = ['_view.xml', '_workflow.xml', '.py']

            # Remove sub-folder in folder 'wizard'
            module_dict['wizard'] = filter(lambda x: '.' in x,
                                           module_dict['wizard'])
            ...
            score_wizard = self.get_score(
                module_dict['wizard'], org_list_wiz, 'wizard/')
            self.count = self.count + 1
        ...
        if 'security' in module_dict:
            ...
            score_security = self.get_score(
                module_dict['security'], security, 'security/')
            self.count += 1
        ...
        self.result = self.get_result(
            {module_name: [module_name, int(self.score * 100)]})
        return None
    ...
    def get_score(self, module_list, original_files, mod_folder=''):
        ...
        for i in module_list:
            if i in original_files:
                ...
                # Do not need to check report folder
                # There are some cases like overwriting a report
                # will not require to put a py file  
                if mod_folder not in ['static/', 'controllers/', 'views', 
                                      'report/']:
                    self.result_dict[i] = [mod_folder + i,
                                           'File name does not follow naming'
                                           ' standards.']
                    score -= 1
                    module_length -= 1
    ...
    def get_result_details(self, dict_struct):
        ...
        header = ('<tr><th class="tdatastyle">%s</th><th class="tdatastyle">'
                  '%s</th></tr>',
                  [_('File Name'), _('Feedback about structure of module')])
        if not self.error:
            res = str_html + \
                self.format_html_table(
                    header, data_list=dict_struct) + '</table></body></html>'
