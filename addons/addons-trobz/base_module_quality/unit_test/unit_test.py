...
class quality_test(base_module_quality.abstract_quality_check):
    ...
    def __init__(self):
        ...
        # self.ponderation = 0
        self.note = _("""
                        This test checks the Unit Test(PyUnit) Cases of the\
                         module. Note that 'unit_test/test.py'\
                         is needed in module.
                    """)
        self.min_score = 0
    ...
    def get_result_details(self, data_list):
        detail = '''<html><head>%s</head><body><table class="tablestyle">
           <tr><th class="tdatastyle">Test Cases</th ><th\
            class="tdatastyle">Result</th>''' % (self.get_style())
        html = ''
        ...
        if data_list[0]:
            ...
            for case in map(lambda x: x[0].replace('\n', ''),
                ...
                if case.find('Ran') != -1:
                    ...
                    html += '<tr><th class="tdatastyle">%s</th><th\
                     class="tdatastyle">OK</th></tr>' % (
                        case)
                else:
                    html += '<tr><td class="tdatastyle">%s</td><td\
                     class="tdatastyle">OK</td></tr>' % (
                        case)
            ...
            for case in map(lambda x: x.split('...'), test):
                ...
                if test_name not in detail_dict:
                    detail_dict[test_name] = ''
                html += '<tr><th class="tdatastyle">%s</th><th\
                 class="tdatastyle">%s</th><td class="tdatastyle">%s</td>\
                 </tr>' % (
                    test_name, case[1], detail_dict[test_name])
