...
class quality_save_report(osv.TransientModel):

    _name = "save.report"
    _description = "Save Report of Quality"

    def default_get(self, cr, uid, fields, context=None):
        super(quality_save_report, self).default_get(
            cr, uid, fields, context=context)
        active_ids = context.get('active_ids')
        if active_ids:
            data = self.pool['module.quality.detail'].browse(
    ...
    _columns = {
        'name': fields.char('File Name', required=True, size=32, help="Save\
         report as .html format"),
