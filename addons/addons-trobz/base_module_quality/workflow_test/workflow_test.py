...
class quality_test(base_module_quality.abstract_quality_check):
    ...
    def __init__(self):
        ...
        self.note = _(
            "This test checks where object has workflow or not on it if there\
             is a state field and several buttons on it and also checks\
              validity of workflow xml file")
    ...
    def run_test(self, cr, uid, module_path):
        ...
        view_ids = view_obj.search(
            cr, uid, [('model', 'in', obj_list), ('type', 'in', ['form'])])
        view_data = view_obj.browse(cr, uid, view_ids)
        field_obj = registry['ir.model.fields']
        field_ids = field_obj.search(cr, uid, [('model', 'in', obj_list)])
        field_data = field_obj.browse(cr, uid, field_ids)
        wkf_obj = registry['workflow']
        wkf_activity_obj = registry['workflow.activity']
        ...
        if obj_list:
            ...
            for i in wkfs:
                ...
                if len(model_ids[i['osv']]) < 2:  # to be modified..
                    bad_view += 1
                    result_dict[i['osv']] = [
                        i['osv'], 'You should have enough demo data which\
                         allows testing of integrity of module and ensures\
                         the proper functioning of workflows']
                else:
                    good_view += 1
        wkf_ids = map(lambda x: x['id'], wkfs)
        if not wkf_ids:
            result_dict[module_name] = [
                module_name, 'No workflow defined on module']
            activity_ids = []
            activities = []
        else:
            # Activity of workflow checking...
            activity_ids = wkf_activity_obj.search(
                cr, uid, [('wkf_id', 'in', wkf_ids)])
        ...
        for activity in activities:
            ...
            if not activity.in_transitions and not activity.out_transitions:
                not_ok += 1
                result_dict[activity.id] = [
                    activity.name, 'Use less activity (improves readability\
        ...
        for act in activity_chk:
            if activity_chk[act]['start'] == 'ok':
                ...
                not_ok += 1
                result_dict[activity_chk[act]['model']] = [activity_chk[act][
                    'model'], 'Workflow activities should have atleast one\
                    starting node']
        ...
        for view in view_data:
            if view.model in state_check:
                ...
                if count > 3 and view.model not in wkf_avail:
                    bad_view += 1
                    result_dict[view.model] = [
                        view.model, 'The presence of a field state in object\
                        often indicative of a need for workflow behind.\
                        And connect them to ensure consistency in this field.']
                elif count > 0 and view.model in wkf_avail:
                    good_view += 1
        score_avail = good_view and float(
            good_view) / float(bad_view + good_view)
        self.score = (score_general + score_avail) / 2
        if self.score * 100 < self.min_score:
            self.message = 'Score is below than minimal\
            score(%s%%)' % self.min_score
    ...
    def get_result(self, dict_wf):
        header = ('{| border="1" cellspacing="0" cellpadding="5"\
        align="left" \n! %-40s \n! %-10s \n',
    ...
    def get_result_details(self, dict_wf):
        ...
        class="tdatastyle">%s</th></tr>',
                  [_('Object Name'), _('Feed back About Workflow of Module')])
        if not self.error:
            res = str_html + \
                self.format_html_table(
                    header,
                    data_list=dict_wf) + '</table><newline/></body></html>'
